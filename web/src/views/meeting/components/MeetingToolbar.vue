  ...
  >
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center gap-2 text-xs">
            <div class="flex items-center gap-1">
              ...
              <span class="text-gray-600 dark:text-gray-400">{{ t('tools.webRtcMeeting.entry.meetingInProgress') }}</span>
            </div>
            <span class="text-gray-400 dark:text-gray-600">•</span>
            <span class="text-gray-600 dark:text-gray-400">{{ participantCount }} {{ t('tools.webRtcMeeting.entry.participants') }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2">
      ...
      <button
        ...
        class="w-9 h-9 rounded-lg bg-white dark:bg-black border border-gray-200 dark:border-gray-800 text-black dark:text-white hover:bg-gray-50 dark:hover:bg-gray-900 flex items-center justify-center shadow-sm transition-all"
        :title="t('tools.webRtcMeeting.entry.roomManagement')"
      >
      ...
      <div class="p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div class="mb-4 sm:mb-6 space-y-3">
          ...
          <div>
            <label class="font-semibold text-black dark:text-white text-xs sm:text-sm mb-2 block">
              {{ t('tools.webRtcMeeting.entry.roomId') }}
            </label>
            <div
        ...
        <div class="flex gap-2">
          ...
          >
            ...
            <span class="sm:hidden">{{ t('tools.webRtcMeeting.entry.copy') }}</span>
          </button>
          <button
            @click="closeModal"
          ...
          >
            ...
            <span class="sm:hidden">{{ t('tools.webRtcMeeting.entry.close') }}</span>
          </button>
        </div>
      </div>
    </div>
...
function handleRoomUpdated() {
  // 房间信息更新后的处理
  toast.success(t('tools.webRtcMeeting.entry.roomInfoUpdated'))
}

function copyLink() {
  // 构建包含房间信息的文本
  let copyText = `${t('tools.webRtcMeeting.entry.inviteToJoin')}\n${t('tools.webRtcMeeting.entry.roomId')}：${roomId.value}\n`
  
  if (roomPassword.value) {
    copyText += `${t('tools.webRtcMeeting.entry.roomPassword')}：${roomPassword.value}\n`
  }
  
  copyText += `\n${t('tools.webRtcMeeting.entry.orDirectLink')}${roomPassword.value ? t('tools.webRtcMeeting.entry.enterPassword') : ''}进入房间：\n${meetingLink.value}`
