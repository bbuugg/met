    ...
    <div class="flex-1 flex flex-col overflow-hidden min-h-0">
      <div class="flex-1 overflow-y-auto p-4 flex flex-col gap-4 scroll-smooth" ref="messagesContainer">
        ...
        <div v-if="chatMessages.length === 0"
          ...
          <h3 class="font-medium text-black dark:text-white mb-2">{{ t('tools.webRtcMeeting.entry.startConversation') }}</h3>
      ...
      <div v-if="unreadMessageCount > 0" @click="scrollToBottomAndMarkRead"
        ...
        <span class="flex items-center gap-2">
          ...
          <span class="font-medium">{{ t('tools.webRtcMeeting.entry.newMessages', { count: unreadMessageCount }) }}</span>
          <span class="opacity-75">{{ t('tools.webRtcMeeting.entry.clickToView') }}</span>
        </span>
      </div>
    </div>
