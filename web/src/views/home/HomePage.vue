  ...
  <div class="fixed top-6 w-full flex justify-between items-center px-6">
    <div class="flex gap-3">
      ...
      <button @click="toggleTheme"
        ...
        :title="currentTheme === 'light' ? t('tools.webRtcMeeting.entry.switchToDarkTheme') : t('tools.webRtcMeeting.entry.switchToLightTheme')">
      ...
      <button @click="toggleLanguage"
        ...
        :title="currentLanguage === 'en-US' ? t('tools.webRtcMeeting.entry.switchToChinese') : t('tools.webRtcMeeting.entry.switchToEnglish')">
        ...
        <template v-if="userStore.info.uuid">
          ...
          <div class="flex bg-gray-100 dark:bg-gray-900 rounded-lg p-1 mb-4">
            <button @click="activeTab = 'create'" :class="[
              ...
              {{ t('tools.webRtcMeeting.entry.createMeetingTab') }}
            </button>
            <button @click="activeTab = 'join'" :class="[
              ...
              {{ t('tools.webRtcMeeting.entry.joinMeetingTab') }}
            </button>
          </div>
          ...
          <div v-show="activeTab === 'create'" class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
              <label for="roomPassword" class="font-semibold text-black dark:text-white text-sm">
                {{ t('tools.webRtcMeeting.entry.roomPassword') }}
              </label>
              <input id="roomPassword" v-model="roomPassword" type="password"
                :placeholder="t('tools.webRtcMeeting.entry.roomPasswordPlaceholder')"
                class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-black text-black dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent transition-all text-sm"
                @keyup.enter="handleCreateAndJoin" />
            </div>
          ...
          <div v-show="activeTab === 'join'" class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
              <label for="joinPassword" class="font-semibold text-black dark:text-white text-sm">
                {{ t('tools.webRtcMeeting.entry.joinPassword') }}
              </label>
              <input id="joinPassword" v-model="joinPassword" type="password"
                :placeholder="t('tools.webRtcMeeting.entry.joinPasswordPlaceholder')"
                class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-black text-black dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent transition-all text-sm"
                @keyup.enter="handleJoin" />
            </div>
        ...
        <div class="flex flex-col gap-6" v-else>
          <div class="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-4">
            <h3 class="font-medium text-black dark:text-white mb-3">
              {{ t('tools.webRtcMeeting.entry.pleaseLoginFirst') }}
            </h3>
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              ...
              <svg v-if="room.hasPassword" class="h-3 w-3 text-gray-500 dark:text-gray-400 flex-shrink-0" fill="none"
                stroke="currentColor" viewBox="0 0 24 24" :title="t('tools.webRtcMeeting.entry.passwordRequired')">
      ...
      <div class="p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div class="flex gap-2">
          <button @click="confirmLogout"
            ...
            <span class="sm:hidden">{{ t('tools.webRtcMeeting.entry.logoutConfirm') }}</span>
          </button>
          <button @click="cancelLogout"
            ...
            <span class="sm:hidden">{{ t('tools.webRtcMeeting.entry.logoutCancel') }}</span>
          </button>
        </div>
      </div>
    </div>
...
const handleJoin = async () => {
  if (isJoinDisabled.value) {
    toast.error(t('tools.webRtcMeeting.errors.enterMeetingId'))
    return
  }
  ...
  try {
    ...
    if (error.response?.status === 401) {
      toast.error(t('tools.webRtcMeeting.errors.wrongPassword'))
    } else if (error.response?.status === 403) {
      toast.error(t('tools.webRtcMeeting.errors.userBlacklisted'))
    } else if (error.response?.status === 404) {
      toast.error(t('tools.webRtcMeeting.errors.roomNotFound'))
    } else {
      toast.error(error.response?.data?.message || t('tools.webRtcMeeting.errors.joinRoomFailed'))
    }
  }
}
...
const handleCreateAndJoin = async () => {
  if (!meetingName.value.trim()) {
    toast.error(t('tools.webRtcMeeting.errors.enterMeetingName'))
    return
  }
  ...
  try {
    ...
    if (response.code === 0) {
      ...
      toast.error(response.data.message || t('tools.webRtcMeeting.errors.createRoomFailed'))
    }
  } catch (error: any) {
    console.error('创建会议失败:', error)
    toast.error(t('tools.webRtcMeeting.errors.createRoomFailed'))
  } finally {
...
const joinRoom = async (roomId: string) => {
  ...
  if (room?.hasPassword) {
    // 如果需要密码，提示用户输入
    const password = prompt(t('tools.webRtcMeeting.entry.enterRoomPassword'))
...
const deleteRoomHandler = async (roomId: string) => {
  try {
    ...
    if (response.code === 0) {
      toast.success(t('tools.webRtcMeeting.success.roomClosed'))
      // 删除成功后刷新房间列表
      await fetchRoomList()
    } else {
      toast.error(response.message || t('tools.webRtcMeeting.errors.closeRoomFailed'))
    }
  } catch (error: any) {
    console.error('关闭房间失败:', error)
    toast.error(t('tools.webRtcMeeting.errors.closeRoomFailed'))
  }
}
</script>
